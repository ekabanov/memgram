...
**Echo cancellation:** `MicrophoneCapture` can enable Apple voice processing (`inputNode.setVoiceProcessingEnabled(true)`) — without AEC, speaker playback bleeds into the mic, makes mic/system energies near-equal, and defeats `selectDominantChannel` (the averaged path mixes a room-delayed echo → garbled transcription). **OPT-IN, default OFF** (`echoCancellationEnabled`, Settings → Recording): the voice-processing unit always ducks "other audio" audibly — even with `duckingLevel: .min` there is no API to fully disable ducking — and the "other audio" is the meeting the user is listening to. Fails soft to plain capture. Read the input format only AFTER configuring voice processing — enabling it changes the node's format. The mic privacy indicator (orange menu-bar pill) is unavoidable TCC UI; it shows "Xcode" for debug launches, "Memgram" in real ones.
...
**LLM backends:** Qwen (local MLX via `mlx-swift-lm`), Custom Server (OpenAI-compatible — covers Ollama/LM Studio/vLLM), Claude, OpenAI, Gemini. `LLMProviderStore.currentProvider` delegates to `providerFor(selectedBackend)`. API keys in Keychain only. All backends stream tokens via `provider.stream()` — cloud providers use SSE, Qwen uses `ChatSession.streamResponse()`. Cloud model IDs are user-editable (`LLMProviderStore.claudeModel`/`openaiModel`/`geminiModel` in UserDefaults; blank = `defaultClaudeModel`/`defaultOpenAIModel`/`defaultGeminiModel` — each vendor's mainstream model, `claude-sonnet-5`/`gpt-5.5`/`gemini-3.5-flash` as of 2026-07, exact IDs without date suffixes). Cloud providers throw typed errors: `LLMNotConfiguredError` when the key is empty (checked before any request) and `LLMAPIError` for non-2xx (parses the shared `{"error":{"message"}}` envelope all three APIs use; special-cases 401/404/429/5xx into actionable messages; streaming paths read the error body off the byte stream). Gemini's key goes in the `x-goog-api-key` header — never the URL query, which leaks into logs. ClaudeProvider sends `thinking: {type: "disabled"}` — Sonnet 5 defaults adaptive thinking ON, and a long transcript then "thinks" for a minute before the first text token, which reads as a stuck summary (guard: Fable/Mythos reject explicit disabled, claude-3.x predates the parameter — those get no thinking key). `SummaryEngine` logs time-to-first-token per streaming request (warns > 20s). Per-meeting engine override: MeetingDetailView's regenerate picker passes `overrideBackend` to `SummaryEngine.summarize` → `providerFor(_:)` — it must never mutate `selectedBackend`.

**Qwen memory lifecycle:** `QwenLocalProvider.preload()` calls `loadModel()` then immediately `unload()` — this downloads and caches model files on disk, momentarily loads weights into memory to verify the model, then frees them. The download progress is reported through the standard `LLMModelFactory` callback so the popover shows a real byte-level progress bar. On `summarize()`, Qwen loads weights on demand; after the summary completes `SummaryEngine` calls `provider.unload()` which calls `MLX.Memory.clearCache()` to return GPU buffers to the OS. This keeps Qwen's peak footprint off RAM except during active use. Model tiers: Qwen 3.5 4B (< 16 GB RAM), Qwen 3.5 9B (16–47 GB), Qwen 3.6 27B dense (≥ 48 GB, ~16 GB weights — preferred over the 35B-A3B MoE after real-world quality comparison). Qwen 3.6 ships no small models, so the lower tiers stay on 3.5.
